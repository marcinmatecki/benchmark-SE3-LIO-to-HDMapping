...
int main(int argc, char **argv)
{
    ...
    for (const rosbag::MessageInstance& m : view) {
        if (m.getTopic() == "/local/cloud_registered_body") {
            ROS_INFO("Received message on topic: /local/cloud_registered_body");
        ...
        if (m.getTopic() == "/local/odometry") {
            ROS_INFO("Received message on topic: /local/odometry");
